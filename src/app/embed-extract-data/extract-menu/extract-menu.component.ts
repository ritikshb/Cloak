...
export class ExtractMenuComponent implements OnInit {
  constructor(
    ...
    private fileTypeService: IdentifyFileTypeService,
  ...
  fileTypes: string[][] = [];
  identifiedExtension: string = "txt";
  hexResult: string;
  asciiResult: string;
  ...
  async startExtract() {
    ...
    */
    ...
    var _pixelOrder: string =
      this.lsbOptions.pixelOrder == "Row" ? "Row" : "Column";
    var _trimBits: boolean = this.lsbOptions.trimBits == "Yes" ? true : false;

    //Run function!
    this.hexResult = await this.extractService.extract(
  ...
  downloadData(): void {
    ...
    */
    ...
    var byteArray: Uint8Array = new Uint8Array(
      this.hexResult.match(/.{2}/g).map((e) => parseInt(e, 16))
    );
    var blob: Blob = new Blob([byteArray], {
    ...
    var downloadName = fileName
      .split(".")
      .slice(0, -1)
      .concat(["." + this.identifiedExtension])
      .join("");
